...
class Model:
    def __init__(self, args):
        ...
        with tf.variable_scope("encoder"):
            ...
            w = tf.constant(1,tf.float32, (1, args.rnn_size, 1))
            encoder_final_state = tf.nn.conv1d(encoder_inputs_embedded, w, stride=1, padding='VALID')
        ...
        def generation(h):
            with tf.variable_scope("generation", reuse=tf.AUTO_REUSE):
                with tf.variable_scope("decoder"):
                    ...
                    #     dtype=tf.float32,
                    # )
                    decoder_init_state = tf.reshape(decoder_init_state,
                                                         [args.batch_size, args.rnn_size, 1])
                    w = tf.constant(1, tf.float32, (1, 1, args.rnn_size+1))
                    decoder_outputs = tf.nn.conv1d(decoder_init_state, w, stride=1, padding='VALID')
                    decoder_outputs = tf.reshape(decoder_outputs, [args.batch_size, args.rnn_size+1, args.rnn_size])
...
def kde_train(data): #data(1079,30)
    ...
    for j in range(len(all_mu)):
        ...
        if len(tp_) > max_mu:
            max_mu = len(tp_)
    mu = np.random.random([len(all_mu),max_mu])
    for k in range(len(all_mu)):
        temp_mu = []
        for m in range(max_mu):
            try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_type', type=str, default="Lym-CNN",
                        help='choose a model')
    ...
    parser.add_argument('--model_id', type=str, default="20",
                        help='model id')
